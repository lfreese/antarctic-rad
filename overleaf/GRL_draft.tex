...
\correspondingauthor{L. M. Freese}{lyssamfreese@gmail.com}
...
\begin{keypoints}
\item We derive an expression for surface temperature change due to a given \ce{CO_2} concentration based on an effective bandwidth and the initial temperature
...
\end{equation}
The sign convention here is positive for each GHE term if CO$_2$ decreases the net longwave energy loss at the top-of-atmosphere or surface, respectively. \add{The model is limited by the use of monthly mean profiles, which does not account for day-to-day variability as well as diurnal patterns. Additionally, parameter choice, such as the surface albedo, are a potential source of bias. As seen in Figure }\ref{fig:sfc_toa_GHE}\add{, a comparison of our surface and top of atmosphere fluxes with monthly observations from CERES between the years 2000 and 2020} \cite{loeb_clouds_2018,kato_surface_2018} \add{shows agreement across all months for the long and short wave fluxes at the surface, and for the long wave top of atmosphere fluxes, with the largest differences appearing in our short wave top of atmosphere fluxes.}  

\begin{figure}[htb!]
\noindent\includegraphics[width=1\textwidth]{GH_sfc_CO2_effect.png}
\centering
\caption{\add{a) }$\text{GHE}_{\text{SFC}}$ (upper) and $\text{GHE}_{\text{TOA}}$ (lower) for a range of \ce{CO_2} concentrations. $\text{GHE}_{\text{SFC}}$ is increasingly positive with higher \ce{CO_2} concentrations, while the responses for $\text{GHE}_{\text{TOA}}$ vary by season. \add{$\text{GHE}_{\text{SFC}}$ is higher during December because of a higher surface temperature, as shown in b) vertical profiles of the initial temperature for each month.}}
\label{fig:sfc_toa_GHE}
\end{figure}

These calculations show that ${\text{GHE}_\text{{SFC}}}$ is always positive, independent of the time of year, while the sign of ${\text{GHE}_\text{{TOA}}}$ is seasonally dependent (Figure \ref{fig:sfc_toa_GHE}). This agrees with the conclusions of previous work  \cite{schmithusen_how_2015}, as we find $\frac{\delta{\text{GHE}_\text{{TOA}}}}{\delta\ce{CO_2}}$ is positive during austral winter months (May-August), but negative throughout the rest of the year (September-April). This increase in OLR with higher \ce{CO_2} concentrations leads to increasing top-of-atmosphere (TOA) radiative loss under higher GHG scenarios during September-April. \add{The forcing is dependent on \ce{CO_2} concentration because of the vertical displacement of the radiating level, which is determined by the lapse rate} \cite{huang_why_2014}. \add{This further explains the negative forcing seen during September-April, as stronger inversions during this time lead to a lapse rate that reverses the sign of the forcing} \cite{huang_inhomogeneous_2016}.
...
A two-way balance between radiative cooling and advective heating can generate equilibrium temperature profiles, but by construction these have zero turbulent heat flux at the surface, no net surface radiative flux, and a discontinuous\add{,but stable} temperature jump between the surface and overlying air \cite{cronin_analytic_2016}\add{(unstable versions of this temperature jump are also seen in} \citeA{manabe_thermal_1964}. Much of the Antarctic plateau has a strong but not discontinuous surface inversion, where surface radiative cooling is balanced by a downward flux of sensible heat from the warmer air above. To capture this transport of heat by boundary-layer turbulence, we include a near-surface turbulent heating component based on the eddy diffusion of potential temperature,
\begin{equation}
    F_{turb} = -k(z) \rho c_p \frac{d\theta}{dz}.
\end{equation}
Here, $k(z) = \overline{k_0} \exp(-\frac{z}{d})$ is an exponentially decaying eddy diffusivity for sensible heat, with a surface value of $k_0$ and a height scale of $d = 100$ meters\add{ (chosen because of the model vertical resolution)}, which confines turbulent fluxes to the bottom \remove{few} model level\remove{s}. To obtain eddy diffusivity values consistent with observed temperature profiles, we first diagnose $k_0$ from the initial temperature profiles for each month $m$,
\begin{equation}
...
This parameterization of turbulent sensible heat fluxes balances the surface radiative heating rate in equilibrium. \add{Given the limitations of a fixed height scale, we compared the results of the model where $d=500$ meters, and found that it did not have a large impact on the temperature response or relevant calculations, as explored below. }
